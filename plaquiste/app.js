...
const CONNECTION_DESTINATIONS = Object.freeze({
  global:Object.freeze({
    ...
    'catalog.optimaClipPricePerMl':'prix catalogue Lisse Clip Optima','catalog.optimaSupportPriceEach':'prix catalogue Appui Optima','catalog.optimaKeyPriceEach':'prix catalogue Clé Optima',
    'catalog.optimaFixingPriceEach':'prix catalogue fixation Optima','catalog.ceilingSplicePriceEach':'prix catalogue éclisse plafond','catalog.ceilingConnectorPriceEach':'prix catalogue cavalier/connecteur plafond',
    'catalog.band30Price':'prix catalogue rouleau bande 30 m','catalog.band50Price':'prix catalogue rouleau bande 50 m','catalog.band150Price':'prix catalogue rouleau bande 150 m',
    'catalog.compound5Price':'prix catalogue pot enduit 5 kg','catalog.compound15Price':'prix catalogue pot enduit 15 kg','catalog.compound25Price':'prix catalogue pot enduit 25 kg'
  }),
...
if(!state.catalog)state.catalog=catalogDefault();
...
function catalogDefault(){return {optimaClipPricePerMl:null,optimaSupportPriceEach:null,optimaKeyPriceEach:null,optimaFixingPriceEach:null,ceilingSplicePriceEach:null,ceilingConnectorPriceEach:null,band30Price:null,band50Price:null,band150Price:null,compound5Price:null,compound15Price:null,compound25Price:null};}
function createInitial(){ return {schemaVersion:'1.3.0',project:{name:'Chiffrage Plaquiste — démo',olderThan2Years:null,energyRenovation:false,eligibilityConfirmed:false},catalog:catalogDefault(),pieces:[makePiece(1)],simpleWalls:[],options:{hourlyRate:null,materialMargin:null,complexity:null,finish:null,impression:false,reprise:false,reprisePrice:R.reprise,access:false,accessPrice:R.access,extras:[]}}; }
...
function initUI(){
  ...
  $('#next').onclick=()=>step===5?renderResults():attemptGo(step+1);
  $('#saveBtn').onclick=()=>save(true);
  $('#calcBtn').onclick=()=>attemptGo(5);
  $('#steps').addEventListener('click',e=>{const b=e.target.closest('[data-step]');if(!b)return;const target=+b.dataset.step;target<=step?go(target):attemptGo(target);});
...
function go(n){navigationErrors=[];step=Math.max(0,Math.min(5,n));render();if(typeof window!=='undefined'&&window.scrollTo)window.scrollTo({top:0,behavior:'smooth'});}
function renderNavigationErrors(){
  if(!content||!navigationErrors.length)return;
  content.insertAdjacentHTML('afterbegin',`<div class="alert err step-block"><b>Étape bloquée :</b> complétez les données obligatoires avant de continuer.</div>${navigationErrors.map(x=>`<div class="alert err step-block"><b>${esc(x.code)}</b> — ${esc(x.message)}</div>`).join('')}`);
}
function render(){ if(!content)return; $('#steps').innerHTML=steps.map((s,i)=>`<button class="step ${i===step?'active':''}" data-step="${i}"><b>${i+1}. ${s[0]}</b><span>${s[1]}</span></button>`).join('');$('#prev').style.visibility=step?'visible':'hidden';$('#next').textContent=step===5?'Recalculer':'Suivant →';[renderProject,renderWorks,renderWalls,renderCeilings,renderOptions,renderResults][step]();renderNavigationErrors(); auditRenderedBindings(content); checkStateStructure(`render-step-${step}`); }
...
function stepError(code,message){return {code,message};}
function validateIsolationForStep(iso,label,errors){
  if(!iso?.active)return;
  if(!iso.reference)errors.push(stepError('PLQ-REQ-ISO-REF',`${label} : choisissez l’isolant.`));
  if(!positive(iso.thickness))errors.push(stepError('PLQ-REQ-ISO-THICKNESS',`${label} : renseignez l’épaisseur de l’isolant.`));
  if(!positive(iso.price))errors.push(stepError('PLQ-REQ-ISO-PRICE',`${label} : renseignez le prix d’achat HT/m².`));
  if(iso.validated!==true)errors.push(stepError('PLQ-REQ-ISO-VALIDATION',`${label} : validez le choix d’isolation.`));
  if(iso.doubleLayer){
    if(!iso.secondReference)errors.push(stepError('PLQ-REQ-ISO2-REF',`${label} : choisissez l’isolant de la deuxième couche.`));
    if(!positive(iso.secondPrice))errors.push(stepError('PLQ-REQ-ISO2-PRICE',`${label} : renseignez le prix de la deuxième couche.`));
  }
}
function validateWallForStep(w,height,label,errors){
  ensureWall(w);if(!w.active)return;
  if(!positive(w.length)||!positive(height)){errors.push(stepError('PLQ-REQ-WALL-DIM',`${label} : longueur et hauteur sont obligatoires.`));return;}
  if(!['cloison','doublage'].includes(w.type))errors.push(stepError('PLQ-REQ-WALL-TYPE',`${label} : choisissez Cloison ou Doublage.`));
  const faces=w.type==='cloison'?[w.face1,w.face2]:[w.face1];
  faces.forEach((f,i)=>{
    if(!PLATES[f.plate])errors.push(stepError('PLQ-REQ-PLATE',`${label} — face ${i+1} : choisissez le type de plaque.`));
    if(typeof f.doubleSkin!=='boolean')errors.push(stepError('PLQ-REQ-SKIN',`${label} — face ${i+1} : choisissez simple ou double peau.`));
    const max=DEMO_CATALOG.plates.at(-1)?.height??null;
    if(max!=null&&Number(height)>max&&!(Number.isInteger(Number(f.manualPlateCount))&&Number(f.manualPlateCount)>0))errors.push(stepError('PLQ-REQ-PLATE-MANUAL',`${label} — face ${i+1} : saisissez le nombre de plaques à commander pour ce mur haut.`));
  });
  if(w.type==='doublage'&&!['classique','optima'].includes(w.framing.system))errors.push(stepError('PLQ-REQ-FRAME-SYSTEM',`${label} : choisissez le système Classique ou Optima.`));
  if(w.type==='cloison'||w.framing.system==='classique'){
    if(!PROFILES[w.framing.profile])errors.push(stepError('PLQ-REQ-PROFILE',`${label} : choisissez le profil d’ossature.`));
    if(![40,60].includes(Number(w.framing.spacingCm)))errors.push(stepError('PLQ-REQ-SPACING',`${label} : choisissez l’entraxe 40 ou 60 cm.`));
    if(typeof w.framing.doubledStuds!=='boolean')errors.push(stepError('PLQ-REQ-STUDS',`${label} : choisissez montants simples ou doublés.`));
  }
  if(w.type==='doublage'&&w.framing.system==='optima'){
    const c=state.catalog||catalogDefault();
    [['optimaClipPricePerMl','Lisse Clip Optima'],['optimaSupportPriceEach','Appui Optima'],['optimaKeyPriceEach','Clé Optima'],['optimaFixingPriceEach','Fixations Optima']].forEach(([k,n])=>{if(!positive(c[k]))errors.push(stepError('PLQ-REQ-CATALOG-OPTIMA',`${label} : renseignez le prix catalogue ${n}.`));});
    const rows=w.framing.optimaRows==null?suggestRows(height):Number(w.framing.optimaRows);if(!(Number.isInteger(rows)&&rows>=1))errors.push(stepError('PLQ-REQ-OPTIMA-ROWS',`${label} : nombre de rangées Optima invalide.`));
  }
  w.openings.forEach((o,i)=>{if(!o.type||!positive(o.width)||!positive(o.height)||!positive(o.qty))errors.push(stepError('PLQ-REQ-OPENING',`${label} : l’ouverture ${i+1} doit avoir type, largeur, hauteur et quantité.`));});
  validateIsolationForStep(w.isolation,label,errors);
  if(Number(w.reinforcementQty)>0&&!positive(w.reinforcementPrice))errors.push(stepError('PLQ-REQ-RENFORT-PRICE',`${label} : renseignez le prix HT du renfort OSB.`));
}
function validateCeilingForStep(p,errors){
  const c=p.ceiling;if(!c?.active)return;ensureCeiling(c);const label=`Plafond ${p.name}`;
  if(!['droit','rampant','rampant_complexe'].includes(c.type))errors.push(stepError('PLQ-REQ-CEILING-TYPE',`${label} : choisissez le type de plafond.`));
  if(['rampant','rampant_complexe'].includes(c.type)&&!positive(c.manualArea))errors.push(stepError('PLQ-REQ-CEILING-AREA',`${label} : renseignez la surface réelle du rampant.`));
  if(!PLATES[c.face.plate])errors.push(stepError('PLQ-REQ-CEILING-PLATE',`${label} : choisissez le parement.`));
  if(typeof c.face.doubleSkin!=='boolean')errors.push(stepError('PLQ-REQ-CEILING-SKIN',`${label} : choisissez simple ou double peau.`));
  if(!DEMO_CATALOG.plates.some(x=>x.height===Number(c.plateFormatHeight)))errors.push(stepError('PLQ-REQ-CEILING-FORMAT',`${label} : choisissez le format de plaque.`));
  if(DEMO_CATALOG.hanger[c.hangerLength]==null)errors.push(stepError('PLQ-REQ-CEILING-HANGER',`${label} : choisissez la longueur de suspente.`));
  const cat=state.catalog||catalogDefault();if(!positive(cat.ceilingSplicePriceEach))errors.push(stepError('PLQ-REQ-CATALOG-SPLICE',`${label} : renseignez le prix catalogue d’une éclisse.`));if(!positive(cat.ceilingConnectorPriceEach))errors.push(stepError('PLQ-REQ-CATALOG-CONNECTOR',`${label} : renseignez le prix catalogue d’un cavalier/connecteur.`));
  validateIsolationForStep(c.isolation,label,errors);
}
function validateStep(index){
  const errors=[];state.catalog=state.catalog||catalogDefault();
  if(index===0){
    if(!String(state.project.name??'').trim())errors.push(stepError('PLQ-REQ-NAME','Le nom du chiffrage est obligatoire.'));
    if(!positive(state.options.hourlyRate))errors.push(stepError('PLQ-REQ-HOURLY','Le taux horaire HT doit être renseigné et supérieur à 0.'));
    if(!nonNegative(state.options.materialMargin))errors.push(stepError('PLQ-REQ-MARGIN','La marge matériaux doit être renseignée (0 % est autorisé).'));
    if(typeof state.project.olderThan2Years!=='boolean')errors.push(stepError('PLQ-REQ-TVA-CONTEXT','Choisissez « logement + de 2 ans » ou « logement - de 2 ans / neuf ».'));
    if(state.project.olderThan2Years===true&&state.project.eligibilityConfirmed!==true)errors.push(stepError('PLQ-REQ-TVA-CONFIRM','Confirmez l’éligibilité au taux réduit avant de continuer.'));
  }else if(index===1){
    if(!state.pieces.length&&!state.simpleWalls.length)errors.push(stepError('PLQ-REQ-WORK','Ajoutez au moins une pièce ou un mur simple.'));
    state.pieces.forEach((p,i)=>{if(!String(p.name??'').trim())errors.push(stepError('PLQ-REQ-PIECE-NAME',`Pièce ${i+1} : le nom est obligatoire.`));if(!positive(p.length)||!positive(p.width)||!positive(p.height))errors.push(stepError('PLQ-REQ-PIECE-DIM',`${p.name||`Pièce ${i+1}`} : longueur, largeur et hauteur sont obligatoires.`));});
    state.simpleWalls.forEach((w,i)=>{if(!String(w.label??'').trim())errors.push(stepError('PLQ-REQ-SIMPLE-NAME',`Mur simple ${i+1} : le nom est obligatoire.`));if(!positive(w.length)||!positive(w.height))errors.push(stepError('PLQ-REQ-SIMPLE-DIM',`${w.label||`Mur simple ${i+1}`} : longueur et hauteur sont obligatoires.`));});
  }else if(index===2){
    const activeWall=state.pieces.some(p=>(p.walls||[]).some(w=>w.active))||state.simpleWalls.some(w=>w.active),activeCeiling=state.pieces.some(p=>p.ceiling?.active);
    if(!activeWall&&!activeCeiling)errors.push(stepError('PLQ-REQ-ACTIVE-WORK','Activez au moins un mur ou un plafond à chiffrer.'));
    state.pieces.forEach(p=>{syncPieceWalls(p);p.walls.forEach(w=>validateWallForStep(w,p.height,`${p.name} — ${w.label}`,errors));});
    state.simpleWalls.forEach(w=>validateWallForStep(w,w.height,w.label,errors));
  }else if(index===3){
    state.pieces.forEach(p=>validateCeilingForStep(p,errors));
  }else if(index===4){
    if(!['aucune','bandes','pret_a_peindre','soignee'].includes(state.options.finish))errors.push(stepError('PLQ-REQ-FINISH','Choisissez le niveau de finition, y compris « Sans finition ».'));
    if(!Object.prototype.hasOwnProperty.call(R.complexity,state.options.complexity))errors.push(stepError('PLQ-REQ-COMPLEXITY','Choisissez la complexité générale du chantier.'));
    if(state.options.reprise&&!positive(state.options.reprisePrice))errors.push(stepError('PLQ-REQ-REPRISE','Renseignez le prix de la reprise sur existant.'));
    if(state.options.access&&!positive(state.options.accessPrice))errors.push(stepError('PLQ-REQ-ACCESS','Renseignez le prix de l’accès difficile.'));
    state.options.extras.forEach((x,i)=>{if(!String(x.name??'').trim()||!positive(x.qty)||!positive(x.price))errors.push(stepError('PLQ-REQ-EXTRA',`Extra ${i+1} : libellé, quantité et prix HT sont obligatoires.`));});
    if(state.options.finish&&state.options.finish!=='aucune'){
      const c=state.catalog;[['band30Price','rouleau de bande 30 m'],['band50Price','rouleau de bande 50 m'],['band150Price','rouleau de bande 150 m'],['compound5Price','pot d’enduit 5 kg'],['compound15Price','pot d’enduit 15 kg'],['compound25Price','pot d’enduit 25 kg']].forEach(([k,n])=>{if(!positive(c[k]))errors.push(stepError('PLQ-REQ-CATALOG-FINISH',`Renseignez le prix catalogue du ${n}.`));});
    }
  }
  trace('STEP-VALIDATE',{index,ok:errors.length===0,count:errors.length,codes:errors.map(x=>x.code)});return errors;
}
function attemptGo(target){
  target=Math.max(0,Math.min(5,Number(target)));if(target<=step){go(target);return true;}
  for(let i=0;i<target;i++){const errors=validateStep(i);if(errors.length){step=i;navigationErrors=errors;trace('STEP-BLOCK',{target,blockedAt:i,errors});render();if(typeof window!=='undefined'&&window.scrollTo)window.scrollTo({top:0,behavior:'smooth'});return false;}}
  navigationErrors=[];step=target;render();if(typeof window!=='undefined'&&window.scrollTo)window.scrollTo({top:0,behavior:'smooth'});return true;
}
...
function wallEditor(w,height,scope,isSimple=false){
  ...
  const optimaCatalogue=isOptima?`<div class="subcard catalog-card"><h3>Prix catalogue Optima obligatoires</h3><p class="hint">Ces prix ne figurent pas dans les données validées fournies : ils restent vides et doivent être renseignés. Aucun prix n’est inventé.</p><div class="grid"><div class="field c3"><label>Lisse Clip HT / ml</label><input class="input" type="number" step=".01" min="0.01" placeholder="Obligatoire" data-path="catalog.optimaClipPricePerMl" value="${val(state.catalog?.optimaClipPricePerMl)}"></div><div class="field c3"><label>Appui HT / unité</label><input class="input" type="number" step=".01" min="0.01" placeholder="Obligatoire" data-path="catalog.optimaSupportPriceEach" value="${val(state.catalog?.optimaSupportPriceEach)}"></div><div class="field c3"><label>Clé HT / unité</label><input class="input" type="number" step=".01" min="0.01" placeholder="Obligatoire" data-path="catalog.optimaKeyPriceEach" value="${val(state.catalog?.optimaKeyPriceEach)}"></div><div class="field c3"><label>Fixation HT / unité</label><input class="input" type="number" step=".01" min="0.01" placeholder="Obligatoire" data-path="catalog.optimaFixingPriceEach" value="${val(state.catalog?.optimaFixingPriceEach)}"></div></div></div>`:'';
  ...
  <div class="subcard"><h3>1. Ossature de ce mur</h3><div class="grid">${systemField}${classicFields}${optimaFields}</div><div class="hint">Suggestion hauteur : ${esc(suggestion.text)}. Elle n’est jamais appliquée silencieusement : tout choix classique reste explicite ; Optima peut garder un nombre de rangées en mode automatique.</div></div>${optimaCatalogue}
...
function renderOptions(){
  const o=state.options,c=state.catalog||catalogDefault();
  const finishCatalog=o.finish&&o.finish!=='aucune'?`<div class="card catalog-card"><h2>Prix catalogue — consommables de finition</h2><p class="hint">Guillaume a validé les conditionnements 30 / 50 / 150 ml pour les bandes et 5 / 15 / 25 kg pour l’enduit. Les prix de ces conditionnements ne sont pas fournis : ils sont obligatoires ici et ne sont jamais inventés. Ces coûts servent à la commande interne et ne sont pas refacturés une seconde fois puisque le forfait finition est un prix de vente complet.</p><div class="grid"><div class="field c4"><label>Bande 30 m — HT / rouleau</label><input class="input" type="number" step=".01" min="0.01" placeholder="Obligatoire" data-path="catalog.band30Price" value="${val(c.band30Price)}"></div><div class="field c4"><label>Bande 50 m — HT / rouleau</label><input class="input" type="number" step=".01" min="0.01" placeholder="Obligatoire" data-path="catalog.band50Price" value="${val(c.band50Price)}"></div><div class="field c4"><label>Bande 150 m — HT / rouleau</label><input class="input" type="number" step=".01" min="0.01" placeholder="Obligatoire" data-path="catalog.band150Price" value="${val(c.band150Price)}"></div><div class="field c4"><label>Enduit 5 kg — HT / pot</label><input class="input" type="number" step=".01" min="0.01" placeholder="Obligatoire" data-path="catalog.compound5Price" value="${val(c.compound5Price)}"></div><div class="field c4"><label>Enduit 15 kg — HT / pot</label><input class="input" type="number" step=".01" min="0.01" placeholder="Obligatoire" data-path="catalog.compound15Price" value="${val(c.compound15Price)}"></div><div class="field c4"><label>Enduit 25 kg — HT / pot</label><input class="input" type="number" step=".01" min="0.01" placeholder="Obligatoire" data-path="catalog.compound25Price" value="${val(c.compound25Price)}"></div></div></div>`:'';
  content.innerHTML=head('Étape 5','Finitions et options chantier','Les options chantier sont distinctes des options propres à chaque mur/plafond.')+
  `<div class="card"><h2>Finition</h2><div class="choice-grid four">${[['aucune','Sans finition'],['bandes','Bandes — 5 €/m²'],['pret_a_peindre','Prêt à peindre — 9 €/m²'],['soignee','Soignée — 13 €/m²']].map(([k,l])=>`<button class="choice-card ${o.finish===k?'selected':''}" data-finish="${k}"><b>${l}</b></button>`).join('')}</div>${['pret_a_peindre','soignee'].includes(o.finish)?`<label class="check top-gap"><input type="checkbox" data-path="options.impression" ${o.impression?'checked':''}> Impression +${R.impression} €/m²</label>`:''}</div>${finishCatalog}<div class="card"><h2>Options chantier</h2><div class="option-list"><div class="option-row"><label class="check"><input type="checkbox" data-path="options.reprise" ${o.reprise?'checked':''}> <b>Reprise sur existant</b></label><input class="input money" type="number" min="0" data-path="options.reprisePrice" value="${o.reprisePrice}"><span>€ HT / chantier</span></div><div class="option-row"><label class="check"><input type="checkbox" data-path="options.access" ${o.access?'checked':''}> <b>Accès difficile</b></label><input class="input money" type="number" min="0" data-path="options.accessPrice" value="${o.accessPrice}"><span>€ HT / chantier</span></div><div class="option-row"><span><b>Complexité générale</b></span><select class="select short" data-path="options.complexity"><option value="" ${!o.complexity?'selected':''}>À choisir</option><option value="simple" ${o.complexity==='simple'?'selected':''}>Simple</option><option value="moyenne" ${o.complexity==='moyenne'?'selected':''}>Moyenne</option><option value="complexe" ${o.complexity==='complexe'?'selected':''}>Complexe</option></select></div></div></div><div class="card"><div class="row between"><div><h2>Extras à prix direct</h2><span class="muted">Aucun temps ni matière automatique n’est inventé.</span></div><button class="btn secondary" data-action="add-extra">+ Ajouter un extra</button></div>${o.extras.map(x=>`<div class="extra-row"><input class="input" data-extra="${x.id}" data-ekey="name" value="${esc(x.name)}"><input class="input" type="number" step="1" min="0" data-extra="${x.id}" data-ekey="qty" value="${val(x.qty)}"><input class="input" type="number" step="1" min="0" data-extra="${x.id}" data-ekey="price" value="${val(x.price)}"><button class="btn danger small" data-action="remove-extra" data-id="${x.id}">×</button></div>`).join('')||`<div class="note">Aucun extra.</div>`}</div>`;
}
...
function resultHtml(r){
  const status=r.blocking.length?`<div class="alert err"><b>Chiffrage incomplet :</b> ${r.blocking.length} point(s) bloquant(s). Un résultat final ne doit pas être utilisé tant que ces points ne sont pas corrigés.</div>`:`<div class="alert ok"><b>Contrôles métier :</b> aucun blocage détecté dans la démo.</div>`;
  return `${status}${r.blocking.map(x=>`<div class="alert err"><b>${esc(x.code)}</b> — ${esc(x.message)}</div>`).join('')}
  <div class="metrics"><div class="metric"><span>Surface parements</span><strong>${fmt(r.surfaces.cladding)} m²</strong></div><div class="metric"><span>Heures</span><strong>${fmt(r.laborHours)} h</strong></div><div class="metric"><span>${r.blocking.length?'Total partiel HT':'Total HT'}</span><strong>${eur(r.totalHT)}</strong></div><div class="metric"><span>${r.blocking.length?'Total TTC non final':'Total TTC'}</span><strong>${eurMaybe(r.totalTTC,'Non calculé')}</strong></div></div>
  ${r.alerts.map(a=>`<div class="alert warn">${esc(a)}</div>`).join('')}
  <details class="details" open><summary>Surfaces par ouvrage</summary><div class="inside table-wrap"><table class="table"><thead><tr><th>Ouvrage</th><th>Type</th><th>Surface 1 face</th><th>Parements</th><th>Isolation</th></tr></thead><tbody>${r.wallRows.map(w=>`<tr><td>${esc(w.label)}</td><td>${esc(w.type)}</td><td>${fmt(w.netOne)} m²</td><td>${fmt(w.cladding)} m²</td><td>${w.isolation?fmt(w.netOne)+' m²':'—'}</td></tr>`).join('')}</tbody></table></div></details>
  <details class="details" open><summary>Matériaux et commandes</summary><div class="inside table-wrap"><table class="table"><thead><tr><th>Désignation</th><th>Qté</th><th>Commande</th><th>PU HT</th><th>Total HT</th></tr></thead><tbody>${r.materials.map(m=>`<tr><td>${esc(m.name)}${m.billable===false?'<div class="internal-badge">Coût interne — non refacturé une 2e fois</div>':''}<div class="hint">${esc(m.note||'')}</div></td><td>${fmt(m.qty)} ${esc(m.unit)}</td><td>${esc(m.order||'—')}</td><td>${m.unitPrice!=null?eur(m.unitPrice):'—'}</td><td>${m.total!=null?eur(m.total):'—'}</td></tr>`).join('')}</tbody></table></div></details>
  <details class="details" open><summary>Main-d’œuvre</summary><div class="inside">${r.labor.map(x=>`<div class="row between line"><span>${esc(x.name)}</span><b>${fmt(x.hours)} h</b></div>`).join('')}<div class="row between total-line"><b>Total MO</b><b>${eur(r.laborCost)}</b></div></div></details>
  <details class="details" open><summary>Vente / forfaits</summary><div class="inside">${r.sales.map(x=>`<div class="row between line"><span>${esc(x.name)}</span><b>${eur(x.total)}</b></div>`).join('')||'<div class="note">Aucun forfait supplémentaire.</div>'}<div class="row between total-line"><b>Total HT</b><b>${eur(r.totalHT)}</b></div><div class="row between line"><span>TVA</span><b>${eurMaybe(r.vat,'Non calculée')}</b></div><div class="row between total-line"><b>Total TTC</b><b>${eurMaybe(r.totalTTC,'Non calculé')}</b></div></div></details>
  <div class="card"><h2>Balises de connexion</h2><p class="muted">${TRACE.length} événements tracés dans cette session. Chaque sélection de mur et chaque écriture de champ est vérifiée après enregistrement.</p></div>`;
}
...
function calculate(){
  ...
  const rawMargin=Number(state.options.materialMargin),rawHourly=Number(state.options.hourlyRate);
  const margin=state.options.materialMargin==null||!Number.isFinite(rawMargin)||rawMargin<0?null:rawMargin/100;
  const hourly=state.options.hourlyRate==null||!Number.isFinite(rawHourly)||rawHourly<=0?null:rawHourly;
  ...
  if(hourly==null)blocking.push({code:'PLQ-CFG-TAUX',message:'Renseignez un taux horaire HT supérieur à 0.'});
  ...
  if(bandMl>0){const cat=state.catalog||catalogDefault(),pack=chooseCheapestPackages(bandMl,[{qty:30,price:cat.band30Price,unit:'m'},{qty:50,price:cat.band50Price,unit:'m'},{qty:150,price:cat.band150Price,unit:'m'}]);if(pack)materials.push({name:'Bandes à joint — coût interne finition',qty:bandMl,unit:'ml',order:`${pack.order} — ${fmt(pack.qty)} ml achetés`,unitPrice:pack.total/bandMl,total:pack.total,billable:false,note:'Coût catalogue interne uniquement : le forfait finition 5 / 9 / 13 €/m² est déjà un prix de vente complet, donc aucune double facturation.',vat:null});else{materials.push({name:'Bandes à joint — besoin interne',qty:bandMl,unit:'ml',order:'prix catalogue requis (rouleaux 30 / 50 / 150 m)',unitPrice:null,total:null,billable:false,note:'Aucun choix de conditionnement n’est inventé.',vat:null});blocking.push({code:'PLQ-V2-007',message:'Commande bandes : renseignez les prix catalogue des rouleaux 30 / 50 / 150 m.'});}}
  if(compoundKg>0){const cat=state.catalog||catalogDefault(),pack=chooseCheapestPackages(compoundKg,[{qty:5,price:cat.compound5Price,unit:'kg'},{qty:15,price:cat.compound15Price,unit:'kg'},{qty:25,price:cat.compound25Price,unit:'kg'}]);if(pack)materials.push({name:'Enduit à joint — coût interne finition',qty:compoundKg,unit:'kg',order:`${pack.order} — ${fmt(pack.qty)} kg achetés`,unitPrice:pack.total/compoundKg,total:pack.total,billable:false,note:'Coût catalogue interne uniquement : déjà compris dans le forfait finition, sans double facturation.',vat:null});else{materials.push({name:'Enduit à joint — besoin interne',qty:compoundKg,unit:'kg',order:'prix catalogue requis (pots 5 / 15 / 25 kg)',unitPrice:null,total:null,billable:false,note:'Aucun choix de conditionnement n’est inventé.',vat:null});blocking.push({code:'PLQ-V2-007',message:'Commande enduit : renseignez les prix catalogue des pots 5 / 15 / 25 kg.'});}}
  ...
  const laborHours=labor.reduce((s,x)=>s+x.hours,0),laborCost=hourly==null?0:laborHours*hourly,materialTotal=materials.reduce((s,x)=>s+(x.billable!==false&&Number.isFinite(x.total)?x.total:0),0),saleTotal=sales.reduce((s,x)=>s+x.total,0),totalHT=materialTotal+saleTotal+laborCost;
  const laborVat=vatFor('normal',blocking),vatIncomplete=laborVat==null||materials.some(x=>x.billable!==false&&Number.isFinite(x.total)&&x.vat==null)||sales.some(x=>Number.isFinite(x.total)&&x.vat==null);
  const vat=vatIncomplete?null:materials.reduce((sum,x)=>sum+(x.billable!==false&&Number.isFinite(x.total)?x.total*x.vat/100:0),0)+sales.reduce((sum,x)=>sum+(Number.isFinite(x.total)?x.total*x.vat/100:0),0)+laborCost*laborVat/100;
  const totalTTC=vat==null?null:totalHT+vat;
...
function addFraming(arr,blocking,alerts,w,height,label,net,margin){
  if(w.type==='doublage'&&w.framing.system==='optima'){
    ...
    const cat=state.catalog||catalogDefault();
    addManualUnitNeed(arr,`Lisse Clip Optima — ${label}`,net*R.optima.clip,R.frameLoss,'ml',cat.optimaClipPricePerMl,margin,'Prix catalogue saisi par l’artisan');
    addManualUnitNeed(arr,`Appuis Optima — ${label}`,net*R.optima.support,R.frameLoss,'u',cat.optimaSupportPriceEach,margin,'Prix catalogue saisi par l’artisan');
    addManualUnitNeed(arr,`Clés Optima — ${label}`,net*R.optima.key,R.frameLoss,'u',cat.optimaKeyPriceEach,margin,'Prix catalogue saisi par l’artisan');
    addManualUnitNeed(arr,`Fixations appuis Optima — ${label}`,net*R.optima.fixingPerRow*rows,R.frameLoss,'u',cat.optimaFixingPriceEach,margin,`${rows} rangée(s) × ${R.optima.fixingPerRow} fixations/m² · prix catalogue saisi`);
    if(!positive(cat.optimaClipPricePerMl)||!positive(cat.optimaSupportPriceEach)||!positive(cat.optimaKeyPriceEach)||!positive(cat.optimaFixingPriceEach))blocking.push({code:'PLQ-CATALOG-PRICE',message:`${label} : prix catalogue Lisse/Appui/Clé/fixations Optima requis ; aucun prix n’est inventé.`});
...
function addCeilingFrame(arr,blocking,alerts,p,c,area,margin){
  ...
  const cat=state.catalog||catalogDefault();addManualUnitNeed(arr,`Éclisses — ${p.name}`,area*R.ceiling.splice,R.frameLoss,'u',cat.ceilingSplicePriceEach,margin,'Prix catalogue saisi par l’artisan');addManualUnitNeed(arr,`Cavaliers / connecteurs — ${p.name}`,area*R.ceiling.connector,R.frameLoss,'u',cat.ceilingConnectorPriceEach,margin,'Prix catalogue saisi par l’artisan');
  if(!positive(cat.ceilingSplicePriceEach)||!positive(cat.ceilingConnectorPriceEach))blocking.push({code:'PLQ-CATALOG-PRICE',message:`Plafond ${p.name} : prix catalogue des éclisses et cavaliers/connecteurs requis ; aucun prix n’est inventé.`});
...
function addManualUnitNeed(arr,name,theoretical,loss,unit,purchaseUnitPrice,margin,note){const qty=theoretical*(1+loss/100),p=Number(purchaseUnitPrice),ok=Number.isFinite(p)&&p>0;arr.push({name,qty,unit,order:ok?`${fmt(qty)} ${unit}`:'Catalogue / prix requis',unitPrice:ok&&margin!=null?p*(1+margin):null,total:ok&&margin!=null?qty*p*(1+margin):null,note:`${note}. Besoin ${fmt(theoretical)} + ${loss} % de perte.`,vat:vatFor('normal',[])});trace('CATALOG-MANUAL-PRICE',{name,purchaseUnitPrice:ok?p:null,margin,connected:ok});}
...
function chooseCheapestPackages(required,packs){
  if(!(Number(required)>0)||!Array.isArray(packs)||!packs.length||packs.some(p=>!positive(p.price)||!positive(p.qty)))return null;
  const min=Math.min(...packs.map(p=>p.qty)),maxCount=Math.ceil(required/min)+2;let best=null;
  const walk=(i,counts,qty,cost,count)=>{if(i===packs.length){if(qty+1e-9<required)return;const c={counts:[...counts],qty,cost,count,surplus:qty-required};if(!best||c.cost<best.cost-1e-9||(Math.abs(c.cost-best.cost)<1e-9&&c.surplus<best.surplus-1e-9)||(Math.abs(c.cost-best.cost)<1e-9&&Math.abs(c.surplus-best.surplus)<1e-9&&c.count<best.count))best=c;return;}for(let n=0;n<=maxCount;n++){counts[i]=n;walk(i+1,counts,qty+n*packs[i].qty,cost+n*packs[i].price,count+n);}};
  walk(0,[],0,0,0);if(!best)return null;const parts=[];best.counts.forEach((n,i)=>{if(n)parts.push(`${n}×${packs[i].qty} ${packs[i].unit||''}`.trim());});return {order:parts.join(' + '),qty:best.qty,total:best.cost,unitPrice:best.cost/best.qty,count:best.count};
}
...
function eur(n){return Number(n||0).toLocaleString('fr-FR',{style:'currency',currency:'EUR'});}function eurMaybe(n,label='Non calculé'){return n==null||!Number.isFinite(Number(n))?label:eur(n);}
...
function runIntegrityTests(){
  ...
  try{
    ...
    test('T57 — toutes les connexions globales, pièce, mur simple et extra passent par leur manifeste',()=>{
      const globalSamples={'project.name':['Test','text'],'options.hourlyRate':[55,'number'],'options.materialMargin':[12.5,'number'],'project.energyRenovation':[true,'checkbox'],'project.eligibilityConfirmed':[true,'checkbox'],'options.impression':[true,'checkbox'],'options.reprise':[true,'checkbox'],'options.reprisePrice':[175,'number'],'options.access':[true,'checkbox'],'options.accessPrice':[475,'number'],'options.complexity':['complexe','select'],'catalog.optimaClipPricePerMl':[1.11,'number'],'catalog.optimaSupportPriceEach':[2.22,'number'],'catalog.optimaKeyPriceEach':[3.33,'number'],'catalog.optimaFixingPriceEach':[0.44,'number'],'catalog.ceilingSplicePriceEach':[0.55,'number'],'catalog.ceilingConnectorPriceEach':[0.66,'number'],'catalog.band30Price':[4.3,'number'],'catalog.band50Price':[6.5,'number'],'catalog.band150Price':[15.2,'number'],'catalog.compound5Price':[7.1,'number'],'catalog.compound15Price':[17.2,'number'],'catalog.compound25Price':[24.4,'number']};
    ...
    test('T77 — étape 1 bloque le passage sans taux horaire obligatoire',()=>{fresh();state.project.olderThan2Years=false;state.options.materialMargin=0;state.options.hourlyRate=null;assert(validateStep(0).some(x=>x.code==='PLQ-REQ-HOURLY'),'Étape 1 laisse passer sans taux horaire');});
    test('T78 — étape 1 bloque le passage sans contexte TVA',()=>{fresh();state.options.hourlyRate=40;state.options.materialMargin=0;state.project.olderThan2Years=null;assert(validateStep(0).some(x=>x.code==='PLQ-REQ-TVA-CONTEXT'),'Étape 1 laisse passer sans contexte TVA');});
    test('T79 — étape 2 bloque une pièce avec dimensions manquantes',()=>{fresh();assert(validateStep(1).some(x=>x.code==='PLQ-REQ-PIECE-DIM'),'Étape 2 laisse passer des dimensions vides');});
    test('T80 — étape 3 bloque un mur actif sans plaque/peau/ossature',()=>{const p=fresh();p.length=4;p.width=3;p.height=2.5;syncPieceWalls(p);p.walls[0].active=true;p.walls[0].type='cloison';const e=validateStep(2);assert(e.some(x=>x.code==='PLQ-REQ-PLATE')&&e.some(x=>x.code==='PLQ-REQ-PROFILE'),'Étape 3 laisse passer un mur incomplet');});
    test('T81 — étape 3 bloque Optima tant que les prix catalogue manquants ne sont pas saisis',()=>{const {w}=validOneWall('doublage');w.framing.system='optima';w.framing.profile=null;w.framing.spacingCm=null;w.framing.doubledStuds=null;const e=validateStep(2);assert(e.filter(x=>x.code==='PLQ-REQ-CATALOG-OPTIMA').length===4,'Prix catalogue Optima manquants non bloquants');});
    test('T82 — étape 4 bloque les accessoires plafond sans prix catalogue',()=>{const p=fresh();p.length=5;p.width=4;p.height=2.5;syncPieceWalls(p);p.ceiling.active=true;p.ceiling.type='droit';p.ceiling.face.plate='BA13';p.ceiling.face.doubleSkin=false;p.ceiling.plateFormatHeight=2.5;p.ceiling.hangerLength=90;const e=validateStep(3);assert(e.some(x=>x.code==='PLQ-REQ-CATALOG-SPLICE')&&e.some(x=>x.code==='PLQ-REQ-CATALOG-CONNECTOR'),'Prix accessoires plafond non bloquants');});
    test('T83 — étape 5 bloque une finition sans prix de conditionnements catalogue',()=>{validOneWall('cloison');state.options.finish='bandes';const e=validateStep(4);assert(e.filter(x=>x.code==='PLQ-REQ-CATALOG-FINISH').length===6,'Prix bandes/enduit manquants non bloquants');});
    test('T84 — TVA non résolue reste null et ne devient jamais 0',()=>{validOneWall('cloison');state.project.olderThan2Years=true;state.project.eligibilityConfirmed=false;const r=calculate();assert(r.vat===null&&r.totalTTC===null,'TVA non résolue convertie en 0');});
    test('T85 — logement neuf calcule réellement la TVA 20 %',()=>{validOneWall('cloison');const r=calculate();assert(r.blocking.length===0&&r.vat>0&&Math.abs(r.vat-r.totalHT*.20)<1e-6&&r.totalTTC>r.totalHT,'TVA 20 % non calculée');});
    test('T86 — main-d’œuvre calculée avec taux horaire + complexité renseignés',()=>{validOneWall('cloison');const r=calculate();assert(r.laborHours>0&&r.laborCost>0,'Main-d’œuvre reste à 0 malgré les paramètres complets');});
    test('T87 — matériaux catalogue connus produisent des prix et totaux',()=>{validOneWall('cloison');const r=calculate();const priced=r.materials.filter(x=>x.billable!==false);assert(priced.length>0&&priced.every(x=>Number.isFinite(x.total)&&x.total>0),'Matériaux catalogue connus restent sans prix');});
    test('T88 — bandes/enduit sont chiffrés en coût interne sans double facturation',()=>{validOneWall('cloison');state.options.finish='bandes';Object.assign(state.catalog,{band30Price:4,band50Price:6,band150Price:15,compound5Price:7,compound15Price:17,compound25Price:25});const r=calculate(),internals=r.materials.filter(x=>x.billable===false);assert(r.blocking.length===0&&internals.length===2&&internals.every(x=>x.total>0),'Coût interne bandes/enduit non calculé');const expected=r.materials.filter(x=>x.billable!==false).reduce((a,x)=>a+(x.total||0),0)+r.sales.reduce((a,x)=>a+x.total,0)+r.laborCost;assert(Math.abs(r.totalHT-expected)<1e-6,'Coût interne refacturé une deuxième fois');});
    test('T89 — les 12 nouveaux prix catalogue possèdent tous une balise de destination',()=>{const keys=Object.keys(catalogDefault());assert(keys.length===12&&keys.every(k=>CONNECTION_DESTINATIONS.global[`catalog.${k}`]),'Balise catalogue manquante');});
    test('T90 — aucune erreur de balise runtime pendant la suite de tests',()=>assert(RUNTIME_ERRORS.length===0,`Erreurs runtime détectées : ${JSON.stringify(RUNTIME_ERRORS)}`));
